...
# Version 1.6
#     Fixed Entry so StringVar uses masterr as master, solves weird
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, root, title="Graphics Window",width=200, height=200, borderwidth=0, autoflush=True):
        global masterr
        masterr = root
        masterr.protocol("WM_DELETE_WINDOW", self.close)

        '''top.XYPositionalData = tk.Canvas(top.XYPositionalDataFrame)
        top.XYPositionalData.place(relx=0.022, rely=0.051, relheight=0.857
                , relwidth=0.959, bordermode='ignore')
        top.XYPositionalData.configure(borderwidth="2")
        top.XYPositionalData.configure(cursor="crosshair")
        top.XYPositionalData.configure(relief="ridge")
        top.XYPositionalData.configure(selectbackground="#c4c4c4")'''

        tk.Canvas.__init__(self, masterr, width=width, height=height, borderwidth=borderwidth)
        #masterr.place(relx=0.022, rely=0.051, relheight=0.857, relwidth=0.959, bordermode='ignore')
        masterr.title(title)
        #self.pack()
        masterr.resizable(0, 0)
    ...
    def __repr__(self):
        if self.isClosed():
            ...
            return "GraphWin('{}', {}, {})".format(masterr.title(),
                                             self.getWidth(),
                                             self.getHeight())
...
class Entry(GraphicsObject):
    ...
    def __init__(self, p, width):
        ...
        #print self.anchor
        self.width = width
        self.text = tk.StringVar(masterr)
        self.text.set("")
        self.fill = "gray"
        self.color = "black"
...
class Image(GraphicsObject):
    ...
    def __init__(self, p, *pixmap):
        ...
        if len(pixmap) == 1: # file name provided
            self.img = tk.PhotoImage(file=pixmap[0], master=masterr)
        else: # width and height provided
            width, height = pixmap
            self.img = tk.PhotoImage(master=masterr, width=width, height=height)
